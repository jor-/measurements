...
def measurements_to_land_sea_mask(land_sea_mask, z_values, t_dim=12):
    ...
    (means, nobs, variances) = measurements.util.regrid.measurements_to_land_sea_mask(measurement_data, land_sea_mask, z_values, t_dim=t_dim, t_range=T_RANGE, x_range=X_RANGE, y_range=Y_RANGE)
    ...
    variances[:,:,:,ANNUAL_THRESHOLD_INDEX:][nobs[:,:,:,ANNUAL_THRESHOLD_INDEX:] < DEVIATION_MIN_MEASUREMENTS / t_dim] = np.inf
