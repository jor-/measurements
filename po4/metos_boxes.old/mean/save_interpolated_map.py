...
from measurements.po4.metos_boxes.mean.io import save_interpolated_map as save
from measurements.po4.metos_boxes.mean.io import get_file
from measurements.po4.metos_boxes.mean.constants import METOS_BOXES_MEANS_INTERPOLATED_MAP_FILE as file



if __name__ == "__main__":
    ## get arguments
    parser = argparse.ArgumentParser(description='Saving interpolated mean map.')
    
    parser.add_argument('-t', '--time_dim', type=int, default=2880, help='Number of time steps.')
    parser.add_argument('-w', '--amount_of_wrap_around', type=float, default=0.1, help='Amount of time wrap around.')
    ...
    with util.logging.Logger(log_file=log_file, disp_stdout=True):
        save(time_dim=time_dim, min_measurements=min_measurements, interpolator_setup=interpolator_setup)
