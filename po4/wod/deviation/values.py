...
class Interpolator(measurements.po4.wod.data.interpolate.Interpolator):
    def __init__(self):
        ...
        super(Interpolator, self).__init__(data_function, CONSTANTS.DEVIATION_DIR, CONSTANTS.INTERPOLATED_DEVIATION_FILENAME)
    
    
    def data_for_points(self):
        return super(Interpolator, self).data_for_points(interpolator_setup=(0.1, 2, 0.2, 1))
    
    def data_for_TMM(self, t_dim):
        return super(Interpolator, self).data_for_TMM(t_dim=t_dim, interpolator_setup=(0.1, 2, 0.2, 0))
    
    def data_for_WOA13(self, t_dim):
        return super(Interpolator, self).data_for_WOA13(t_dim=t_dim, interpolator_setup=(0.1, 2, 0.2, 0))
    
    def data_for_WOA13R(self, t_dim):
        return super(Interpolator, self).data_for_WOA13R(t_dim=t_dim, interpolator_setup=(0.1, 2, 0.2, 0))
