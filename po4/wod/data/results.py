...
class Measurements_Unsorted():
    ...
    def iterate(self, fun, minimum_measurements=1, return_type='array'):
        ...
        if return_type is 'array':
            ...
            logger.debug('{} values calculted.'.format(values.shape[0]))
...
# #         results_with_shifted_iterable = itertools.chain([itertools.product(result_list, result_shifted_list) for (result_list, result_shifted_list) in measurements_dict_list])
# 
# #         logger.debug('Results %d gathered.' % len(results_with_shifted_list))
#         logger.debug('Results gathered.')
#         
# #         return results_with_shifted_iterable
#         return measurements_dict_list
    
#     
#     
#     
#     def filter(self, lower_bound, upper_bound):
#         logger.debug('Filtering results with lower bound lower_bound and upper bound %d.' % (same_bounds, minimum_measurements))
#         
#         ## save measurements dict
#         measurements_dict = self.measurements_dict
#         
...
class Measurements(Measurements_Unsorted):
...
#                     logger.log(logging.DEBUG + 10, 'No values in key range found.')
    ...
    def save(self, file=MEASUREMENTS_DICT_FILE):
        Measurements_Unsorted.save(self, file)
    
    
    def load(self, file=MEASUREMENTS_DICT_FILE):
        Measurements_Unsorted.load(self, file)
