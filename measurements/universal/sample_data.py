...
class SampleCorrelationMatrix:
    ...
    def __init__(self, measurements, sample_lsm, min_measurements, min_abs_correlation=measurements.universal.constants.CORRELATION_MIN_ABS_VALUE, max_abs_correlation=measurements.universal.constants.CORRELATION_MAX_ABS_VALUE, dtype=np.float32, matrix_format='csc'):
...
class SampleCorrelationMatrixCache(SampleCorrelationMatrix):
    ...
    def map_indices_to_point_index_dict_cache_file(self, discard_year=False):
        return measurements.universal.constants.MAP_INDEX_TO_POINT_INDEX_DICT_FILE.format(
            ...
            discard_year=discard_year)

    def concentrations_same_points_except_year_dict_cache_file(self):
        return measurements.universal.constants.CONCENTRATIONS_SAME_POINTS_EXCEPT_YEAR_DICT_FILE.format(
            tracer=self.measurements.tracer,
            data_set=self.measurements.data_set_name,
            sample_lsm=self.sample_lsm,
            min_measurements_correlation=self.min_measurements)

    def sample_covariance_dict_cache_file(self):
        return measurements.universal.constants.SAMPLE_COVARIANCE_DICT_FILE.format(
            tracer=self.measurements.tracer,
            data_set=self.measurements.data_set_name,
            sample_lsm=self.sample_lsm,
            min_measurements_correlation=self.min_measurements)
    ...
    def same_box_correlation_matrix_lower_triangle_cache_file(self):
        return measurements.universal.constants.SAMPLE_CORRELATION_MATRIX_SAME_BOX_LOWER_TRIANGLE_MATRIX_FILE.format(
            tracer=self.measurements.tracer,
            data_set=self.measurements.data_set_name,
            sample_lsm=self.sample_lsm,
            min_abs_correlation=self.min_abs_correlation,
            standard_deviation_id=self.measurements.standard_deviation_id_without_sample_lsm,
            dtype=self.dtype,
            matrix_format=self.matrix_format)

    def different_boxes_quantity_lower_triangle_matrix_cache_file(self):
        return measurements.universal.constants.SAMPLE_QUANTITY_MATRIX_DIFFERENT_BOXES_LOWER_TRIANGLE_MATRIX_FILE.format(
            tracer=self.measurements.tracer,
            data_set=self.measurements.data_set_name,
            sample_lsm=self.sample_lsm,
            min_measurements_correlation=self.min_measurements,
    ...
    def different_boxes_correlation_lower_triangle_matrix_cache_file(self):
        return measurements.universal.constants.SAMPLE_CORRELATION_MATRIX_DIFFERENT_BOXES_LOWER_TRIANGLE_MATRIX_FILE.format(
            tracer=self.measurements.tracer,
            data_set=self.measurements.data_set_name,
            sample_lsm=self.sample_lsm,
            min_measurements_correlation=self.min_measurements,
    ...
    def correlation_matrix_cache_file(self):
        return measurements.universal.constants.SAMPLE_CORRELATION_MATRIX_FILE.format(
            tracer=self.measurements.tracer,
            data_set=self.measurements.data_set_name,
            sample_lsm=self.sample_lsm,
            min_measurements_correlation=self.min_measurements,
